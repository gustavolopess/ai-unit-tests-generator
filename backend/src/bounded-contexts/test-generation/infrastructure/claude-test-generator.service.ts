...
} from '@/bounded-contexts/test-generation/domain/services/test-generator.interface';

@Injectable()
export class ClaudeTestGeneratorService implements ITestGenerator {
  private readonly logger = new Logger(ClaudeTestGeneratorService.name);
  ...
  async generateTests(
    ...
    try {
      this.logger.log(
        `Generating tests for ${targetFilePath} in ${workingDirectory}`,
      );
      ...
      const result = await claude()
        ...
        .onMessage((msg) => {
          ...
          if (msg.type === 'assistant') {
            ...
            const writeToolCalls = msg.content.filter(
              (block: any) =>
